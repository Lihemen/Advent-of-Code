...
function testString(array) {
	 ...
	 * @param {String} string
	 */
	function characterCount(min, max, char, string) {
		let chars = string.split('').filter((el) => el == char);
		if (chars.length < min) {
			return counter;
		} else if (chars.length >= min && chars.length <= max) {
			return (counter = counter + 1);
		} else return counter;
	}
	arr.forEach((word) => {
		let stringToTest = word.split(':')[1].trim();
		let char = word.split(':')[0].split(' ')[1];
		let min = word.split(':')[0].split(' ')[0].split('-')[0];
		let max = word.split(':')[0].split(' ')[0].split('-')[1];

		characterCount(min, max, char, stringToTest);
	});
	console.log(counter);
	return counter;
}
...
function checkValidity(array) {
	...
	function check(start, end, char, string) {
		if (string.charAt(start - 1) == string.charAt(end - 1)) {
			return false;
		} else if (
			string.charAt(start - 1) === char ||
			string.charAt(end - 1) === char
		) {
	...
	arr.forEach((word) => {
		let stringToTest = word.split(':')[1].trim();
		let char = word.split(':')[0].split(' ')[1];
		let start = word.split(':')[0].split(' ')[0].split('-')[0];
		let end = word.split(':')[0].split(' ')[0].split('-')[1];

		check(start, end, char, stringToTest);
	});
	console.log(valid);
	return valid;
}
